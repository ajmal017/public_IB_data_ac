...
def getLatestEpoch(curs, ticker, whichTable):
    assert(whichTable == "bars_5_sec" or whichTable == "bars_15_min" or whichTable == "bars_daily")
    cmd = "SELECT epoch FROM " + whichTable + " WHERE ticker='" + ticker + "' ORDER BY epoch DESC;"
    curs.execute(cmd)
...
class TestApp(TestWrapper, TestClient):
    ...
    def chooseStocksHelper(self, ticker, undContract):
        ...
        self.minEpochs[ticker] = getLatestEpoch(self.postgresCursor, ticker, "bars_15_min")
    ...
    def historicalDataRequests_req(self, numDays=365):
        ...
        for m in self.dataIds:
            ...
            self.reqHistoricalData(m, self.underlyingContract[m], "", "1 W", sizeMin(15), "MIDPOINT", 1,1,False,[])
    ...
    def historicalData(self, reqId:int, bar: BarData):
        ...
        if epoch > self.minEpochs[ticker]:
            insertOneBar(self.postgresConnection, bar, epoch, ticker, whichTable="bars_15_min")
